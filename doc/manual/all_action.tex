...
\begin{itemize}
  ...
  \item \verb|'用途'| 枚举类型，包括\verb|"无用途"|，\verb|"用于使用"|，\verb|'用于响应'|，\verb|'用于弃牌'|，\verb|'用于重铸'|，默认为\verb|'无用途'|。用于使用和用于响应功能上等同于下面两个“选牌使用”和“选牌打出”。
  \item \verb|'目标'| 玩家类型，默认为空。
  \item \verb|'是否为改判'| 布尔类型，默认为假。
  \item \verb|'技能名'| 字符串类型，默认为空。
  \item \verb|'是否为临时'| 布尔类型，默认为假。
\end{itemize}
...
\begin{itemize}
  \item \verb|'位置'| 字符串类型，表示可以被选牌的区域，默认为手牌。
  \item \verb|'原因'| 字符串类型，表示被选牌的原因，默认为空。
  \item \verb|'是否可见手牌'| 布尔类型，默认为假。
  \item \verb|'用途'| 枚举类型，包括\verb|"无用途"|，\verb|"用于使用"|，\verb|'用于响应'|，\verb|'用于弃牌'|，\verb|'用于重铸'|，默认为\verb|'无用途'|。
  \item \verb|'禁止弃置列表'| 牌列表类型，禁止弃置的牌的列表，默认为空。
\end{itemize}